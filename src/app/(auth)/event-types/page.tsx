      ...
      <div className="flex">
        ...
        <main className="flex-1 md:ml-72 p-4 pt-20 md:p-10 md:pt-10">
          ...
          <header className="sticky top-0 -mx-10 px-10 py-6 bg-slate-50/80 backdrop-blur-md z-20 flex justify-between items-end mb-8">
            <div>
              <h1 className="text-3xl font-bold text-slate-900 tracking-tight mb-1">予約メニュー</h1>
              <p className="text-slate-500 font-medium">予約可能なイベントを管理</p>
            </div>
            <Link href="/event-types/new">
              <button className="flex items-center gap-2 px-6 py-3.5 rounded-full text-white font-bold text-sm shadow-xl shadow-brand-500/30 hover:shadow-brand-500/50 hover:scale-105 transition-all active:scale-95 bg-gradient-to-r from-brand-500 to-brand-600">
          ...
          {eventTypes.length === 0 ? (
            ...
            <div className="space-y-4">
              {eventTypes.map((eventType) => (
                <div
                  key={eventType.id}
                  className="bg-white rounded-2xl shadow-[0_10px_30px_-10px_rgba(0,0,0,0.05)] border border-slate-100 p-6 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] transition-all duration-300"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`w-3 h-3 rounded-full ${eventType.is_active ? 'bg-brand-500' : 'bg-slate-300'}`} />
                      <div>
                        <h3 className="font-bold text-lg text-slate-900">{eventType.title}</h3>
                        {eventType.description && (
                          <p className="text-sm text-slate-500 mt-0.5">{eventType.description}</p>
                        )}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Link href={`/book/${eventType.slug}`} target="_blank">
                        <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-100 text-slate-700 text-sm font-semibold hover:bg-slate-200 transition-all">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <Link href={`/event-types/${eventType.id}`}>
                        <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl border-2 border-slate-200 text-slate-700 text-sm font-semibold hover:border-brand-500 hover:text-brand-600 transition-all">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
