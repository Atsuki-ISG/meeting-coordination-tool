...
3. 以下の項目を入力:
   ...
   - **説明**: 予約の詳細説明（任意）
   - **所要時間**: 15分/30分/45分/60分/90分/120分のプリセット、または「カスタム」で5〜480分の任意の時間を指定
   - **参加モード**: 「全員必須」（全員が空いている時間のみ）または「誰か空いていれば」から選択
   - **参加者の決定方法**（「誰か空いていれば」のときのみ表示）:
     - **均等**: 過去30日間の担当件数が最も少ないメンバーを自動で割り当てます。担当件数をチーム内で均等に近づけたい場合に使用します
     - **優先度順**: メンバーリストで並び替えた順に、空いている最上位のメンバーが担当になります。並び替えは各メンバーの ↑↓ ボタンで行い、上にあるほど優先されます
   - **参加メンバー**: この予約に参加するメンバーを選択（全員から選択可能）。「優先度順」を選んだ場合は上から順に優先度が高くなります
   - **議事録担当**: チーム設定で「議事録担当」に指定されたメンバーから選択。カレンダー招待に自動追加されます（任意）
   - **カレンダータイトル**: チームメンバーのGoogleカレンダーに表示されるイベントのタイトルをカスタマイズできます。変数（`{予約者名}`, `{メニュー名}`, `{日付}`, `{時刻}`, `{メール}`, `{備考}`）をクリックして挿入でき、プレビューで確認できます。ゲストには表示されません。
   - **ゲスト向けタイトル**: 予約者のGoogleカレンダーに表示されるイベントタイトルをカスタマイズできます。変数（`{予約者名}`, `{メニュー名}`, `{会社名}`, `{日付}`, `{時刻}`, `{メール}`, `{備考}`, `{meet_link}`）を使用できます。デフォルト: `{メニュー名}`
   - **ゲスト向けメモ欄**: 予約者のGoogleカレンダーに表示されるイベント説明をカスタマイズできます。同じ変数が使用でき、プレビューで確認できます。デフォルト: `{meet_link}`（Google MeetリンクのURL）
   - **予約可能時間帯の制限**: メンバーの可用性設定に加えて、この予約タイプ専用の時間帯制限を設定できます
     - **制限なし**: メンバーの可用性設定のみで判定
     - **テンプレートを使用**: チーム設定で作成した時間帯テンプレートを適用
     - **カスタム設定**: 曜日と時間帯を個別に指定
   - **予約を受け付ける**（トグル）: 無効にすると予約ページにアクセスできなくなります（デフォルト: 有効）
   - **詳細設定**（折りたたみ）:
     - **公開期間**: 何日先まで予約可能にするか（1〜90日、デフォルト14日）
     - **最低予約猶予**: 直前すぎる予約を防ぐ時間（なし/30分/1時間/2時間/4時間/8時間/24時間、デフォルト1時間）
     - **前後のバッファ**: 予定の前後に確保する余裕時間（なし/5分/10分/15分/30分/60分、デフォルトなし）
4. 「作成」ボタンをクリック
...
2. 以下の操作が可能:
   - 承認待ちユーザーの承認/拒否
   - 全ユーザーのステータス管理（有効化/停止）
   - **チームへの追加**: 各ユーザーの「チーム設定」ボタンからチームとロールを指定して追加。チームに追加すると同時にアカウントが有効化される
   - 各ユーザーの所属チーム一覧の確認
   - システム管理者権限の付与/解除
   - API利用量の確認
   - メンテナンスモードの切り替え
...
- 各日付には以下の情報が表示されます:
  - **時間帯**: その日に予約できる最早〜最遅の開始時刻（例: 09:00〜16:00）
- 空きがない日は「ー」と表示されます
- 前月/次月への移動が可能です
- 日付をクリックすると 15分間隔の時間枠一覧が表示されます（例: 09:00, 09:15, 09:30…）
...
**確定画面で表示される情報**:
- 予約日時
- Google Meetリンク（クリックでそのまま会議に参加可能）
- キャンセルURL

**重要**: Google Meetリンクはカレンダー招待にも含まれています。キャンセルURLは後で使用するため、控えておいてください。
